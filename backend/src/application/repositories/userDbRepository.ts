...
export const userDbRepository = (
    ...
    const getUserByPhoneNumber = async (phoneNumber: number) =>
        await repository.getUserByPhoneNumber(phoneNumber)

    const getUserById = async (id: string) => await repository.getUserById(id)

    const updateUserByProperty = async (id: string, update: object) =>
        await repository.updateUserByProperty(id, update)

    const getAllUsers = async () => repository.getAllUsers()

    const changeUserRole = async (id: string, GSTNumber: string) =>
        await repository.changeUserRole(id, GSTNumber)

    const changeProfileImg = async (id: string, url: string) =>
        await repository.changeProfileImg(id, url)
